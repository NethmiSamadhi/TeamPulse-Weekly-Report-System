...
export async function seedReports(
      ...
      versions: [
        {
          ...
          deliverable:
            "Login and registration API implementation",
          actualPercentage: 85,
          submittedAt: new Date(
          ...
          blocker:
            "Test environment configuration was delayed.",
        },
        {
          versionNumber: 2,
          taskName:
            "Develop and test authentication API",
          deliverable:
      ...
      versions: [
        {
          versionNumber: 1,
          taskName:
            "Create manager dashboard filters",
          deliverable:
            "Reusable date, user and project filter components",
          actualPercentage: 90,
          submittedAt: new Date(
            "2026-09-03T09:15:00.000Z",
          ),
          blocker:
            "Waiting for final dashboard metric definitions.",
        },
      ],
    },
    {
      ...
      versions: [
        {
          ...
          reviewComment:
            "Please add the missing test coverage figures and clarify the unresolved blocker.",
          blocker:
            "Two API test cases are failing intermittently.",
        },
      ],
    },
    {
      ...
      versions: [
        {
          versionNumber: 1,
          taskName:
            "Research AI-assisted report summaries",
          deliverable:
  ...
  // Remove old demonstration activities and reports.
  // This is safe only for local development seed data.
  ...
  for (const reportData of reports) {
    ...
    const createdReport =
      await prisma.weeklyReport.create({
        data: {
          ...
          dueDate: reportData.weekEnd,
          status: reportData.status,
          currentVersionNumber:
            reportData.currentVersionNumber,
          latestReviewerComment:
            reportData.latestReviewerComment,
          submittedAt: reportData.submittedAt,
          approvedAt: reportData.approvedAt,

          versions: {
            create: reportData.versions.map(
              (version) => ({
                versionNumber:
                ...
                nextWeekTasks: {
                  create: [
                    {
                      taskName:
                        "Complete the next planned development milestone",
                      priority:
                        TaskPriority.MEDIUM,
                      notes:
                        "Coordinate requirements with the project team.",
                      displayOrder: 1,
                    },
                  ],
                },
                ...
                achievements: {
                  create: [
                    {
                      description:
                        `Delivered progress on: ${version.taskName}`,
    ...
    if (
      ...
      activityAction = "REPORT_APPROVED";
    }
  ...
  console.log(
    `Weekly reports created: ${reports.length}`,
  );
}